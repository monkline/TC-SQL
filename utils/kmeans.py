...
def get_kmeans_centers(bert, tokenizer, train_loader, num_classes, max_length):
    for i, (batch, selected) in enumerate(train_loader):
        ...
        corpus_embeddings = get_mean_embeddings(
            bert, tokenized_features['input_ids'], tokenized_features['attention_mask']
            )
    ...
    print(
        "Iterations:{}, Clustering ACC:{:.3f}, centers:{}".format(
            clustering_model.n_iter_, confusion.acc(), clustering_model.cluster_centers_.shape
            )
        )
