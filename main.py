...
def get_args(argv):
    ...
    parser.add_argument('--bert', type=str, default='bge-base-en-v1.5', help="")
    ...
    if args.gpuid[0] >= 0:
        ...
        if torch.backends.mps.is_available(): args.device = torch.device("mps")
        if torch.cuda.is_available(): args.device = torch.device("cuda")
